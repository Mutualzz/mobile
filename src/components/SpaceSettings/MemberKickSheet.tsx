...
export const MemberKickSheet = observer(({ visible, onClose, space, member }: Props) => {
        ...
        <Modal visible={visible} animationType="fade" transparent>
            <Box style={{ flex: 1, justifyContent: "center", padding: 24, backgroundColor: "rgba(0,0,0,0.45)" }}>
                <Paper style={{ padding: 20, borderRadius: 12, gap: 12 }}>
                    <Typography weight="bold">Kick {member.user?.displayName}?</Typography>
                    <InputDefault fullWidth placeholder="Reason (optional)" value={reason} onChangeText={setReason} />
                    {error && <Typography color="danger" level="body-sm">{error}</Typography>}
                    <Button color="danger" disabled={isPending} onPress={() => mutate()}>Kick</Button>
