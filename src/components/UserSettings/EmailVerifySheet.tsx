        ...
        <Modal visible={visible} animationType="slide" transparent>
            <Box style={{ flex: 1, justifyContent: "flex-end", backgroundColor: "rgba(0,0,0,0.45)" }}>
                <Paper style={{ borderTopLeftRadius: 16, borderTopRightRadius: 16, padding: 20, gap: 12 }}>
                    <Typography level="body-lg" weight="bold">Verify Email</Typography>
                    <InputDefault fullWidth placeholder="Verification code" value={code} onChangeText={setCode} />
                    {error && <Typography color="danger" level="body-sm">{error}</Typography>}
                    <Button variant="soft" disabled={sendingCode} onPress={() => sendCode()}>
                        {sendingCode ? "Sending..." : "Send verification code"}
                    </Button>
                    <Button disabled={isPending} onPress={() => mutate()}>Verify</Button>
