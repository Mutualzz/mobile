...
export class SpaceMember {
  ...
  constructor(
    ...
    if (member.user) {
      ...
      if (existing) {
        ...
        this._user = this.app.users.add(member.user);
      }
    }
  ...
  update(member: APISpaceMember) {
    ...
    if (member.user) {
      ...
      if (existing) this._user = existing;
  ...
  resolveChannelPermissions(channel: Channel): BitField<PermissionFlags> {
    const space = this.space;
    if (!space || space.roles.all.length === 0) {
  ...
  canManageMember(target: SpaceMember, permission: PermissionFlag): boolean {
    ...
    if (space.ownerId === this.userId) return true;

    if (space.ownerId === target.userId) return false;
